...
export class DataService {
  ...
  getOffers(): Observable<Offer[]> {
    return this.http.get<Offer[]>(this.apiBaseUrl + 'api/user/offers');
  }

  getOffer(offerId: number): Observable<Offer> {
    return this.http.get<Offer>(this.apiBaseUrl + 'api/user/offer', {
      params: { id: offerId }
    });
  }
  ...
  async generateFileHash(file: globalThis.File): Promise<string | null> {
    let hash = await file.arrayBuffer().then((data: ArrayBuffer) => {
      let hash = sha256.hash(new Uint8Array(data));
      return Buffer.from(hash).toString('hex'); // Convert Uint8Array to string
    });
